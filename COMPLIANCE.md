...
| Schemathesis `allow_header_conformance` (schemathesis >= 4.25)                  | `OPTIONS /collections/{id}/{position,area}` returns 405 with `Allow: GET`, omitting the documented POST; the OGC schema's cube POST (unimplemented) is also reported  | Check excluded in the fuzz test with a comment. GET and POST are separate routes and Starlette builds `Allow` from only the first route matching the path; correcting it needs a 405 middleware on the composed app (xpublish / xpublish-ogc-core), which a plugin cannot install  |
